...
function KitchenScene() {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      {/* Window */}
      <div style={{ position: "absolute", left: "30%", top: "5%", width: "38%", height: "28%", backgroundColor: "#C8DCE8", border: "8px solid #E8D4C0", borderRadius: "2px" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse, rgba(255,240,160,0.25) 0%, transparent 70%)" }} />
      </div>
      {/* Backsplash */}
      <div style={{
      ...
      {/* Floor */}
      <div style={{
        position: "absolute", left: 0, top: "70%", right: 0, bottom: 0,
        background: "repeating-linear-gradient(90deg, #C49A70 0px, #C49A70 98px, #B8896A 98px, #B8896A 100px)",
      }} />
...
function LivingRoomScene() {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      {/* Window */}
      <div style={{ position: "absolute", left: "10%", top: "8%", width: "25%", height: "38%", backgroundColor: "#C8D8E8", border: "8px solid #E0D4C8" }}>
...
function BackyardScene() {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      {/* Sky */}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, #C8DCEC 0%, #E8F0F8 60%)" }} />
      {/* Clouds */}
      <div style={{ position: "absolute", left: "10%", top: "8%", width: "18%", height: "8%", backgroundColor: "rgba(255,255,255,0.7)", borderRadius: "50px" }} />
      <div style={{ position: "absolute", left: "65%", top: "12%", width: "14%", height: "6%", backgroundColor: "rgba(255,255,255,0.6)", borderRadius: "50px" }} />
      {/* Grass */}
      <div style={{ position: "absolute", left: 0, top: "55%", right: 0, bottom: 0, background: "linear-gradient(to bottom, #8FB87A, #7AA068)" }} />
      <div style={{ position: "absolute", left: 0, top: "53%", right: 0, height: "6%", background: "radial-gradient(ellipse 100% 50% at 50% 0%, #9AC87A, transparent)" }} />
      {/* Fence */}
      ...
      <div style={{ position: "absolute", right: "9%", top: "12%", width: "16%", height: "28%", backgroundColor: "#6A9A58", borderRadius: "50%" }} />
      {/* Sunny patch */}
      <div style={{ position: "absolute", left: "50%", top: "62%", width: "20%", height: "12%", background: "radial-gradient(ellipse, rgba(255,230,100,0.35), transparent 70%)" }} />
    </div>
  );
}
...
function CafeScene() {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      {/* Window */}
      <div style={{ position: "absolute", left: "5%", top: "8%", width: "28%", height: "40%", backgroundColor: "#8AAAB8", border: "10px solid #5C4A3C", borderRadius: "2px" }}>
      ...
      {/* Pendant light */}
      <div style={{ position: "absolute", left: "47%", top: "0", width: "2px", height: "30%", backgroundColor: "#5C4A3C" }} />
      <div style={{ position: "absolute", left: "43%", top: "28%", width: "10%", height: "8%", backgroundColor: "#C4A97D", borderRadius: "0 0 50% 50%", boxShadow: "0 4px 20px rgba(196,169,125,0.4)" }} />
      {/* Floor */}
...
const SCENE_BG: Record<SceneId, () => React.JSX.Element> = {
...
// ─── Hotspot component with glow ──────────────────────────────────────────────

function Hotspot({
  def, index, scene, hasMemory, discovered, active, preview, onClick,
}: {
  def: HotspotDef; index: number; scene: SceneId; hasMemory: boolean; discovered: boolean; active: boolean; preview: boolean; onClick: () => void;
}) {
  return (
    <div
      onClick={hasMemory && !preview ? onClick : undefined}
      style={{
        position: "absolute",
        left: `${def.x}%`,
        top: `${def.y}%`,
    ...
    >
      ...
      {hasMemory && !discovered && (
        <>
          <motion.div
            animate={{ scale: [1, 1.5, 1], opacity: [0.2, 0.55, 0.2] }}
            transition={{ duration: 3, repeat: Infinity, delay: index * 0.55, ease: "easeInOut" }}
            style={{
              position: "absolute",
              inset: "-10px",
              borderRadius: "50%",
...
function MemoryCard({ memory, label, onClose }: { memory: Memory; label: string; onClose: () => void }) {
  return (
    <>
      <motion.div
      ...
      >
        {memory.photo && (
          <div style={{ width: "100%", maxHeight: "200px", overflow: "hidden", borderRadius: "1rem 1rem 0 0" }}>
            {memory.photo.url ? (
...
function CompletionScreen({ creation, onClose }: { creation: TributeCreation; onClose: () => void }) {
  return (
    <motion.div
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      style={{ position: "fixed", inset: 0, backgroundColor: "rgba(42,36,32,0.6)", backdropFilter: "blur(6px)", zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}
      onClick={onClose}
    >
      <motion.div
        ...
        style={{ backgroundColor: "#FAF7F4", borderRadius: "1rem", padding: "2.5rem 2rem", maxWidth: "400px", width: "100%", textAlign: "center" }}
      >
        <p style={{ fontFamily: "var(--font-serif)", fontSize: "1.875rem", color: "#C4A97D", margin: "0 0 1rem" }}>You found them all.</p>
        {creation.dedicationMessage && (
          <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "1.125rem", color: "#2A2420", lineHeight: 1.6, margin: "0 0 1.5rem" }}>{creation.dedicationMessage}</p>
        )}
        <div style={{ borderTop: "1px solid #EDE7DF", paddingTop: "1rem" }}>
          <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.8125rem", color: "#8B7D72", margin: "0 0 0.5rem" }}>Made with Chère</p>
          <a href="https://chere.app" style={{ fontFamily: "var(--font-serif)", fontSize: "0.9375rem", color: "#C4A97D" }}>Make one for someone you love →</a>
        </div>
      </motion.div>
    </motion.div>
  );
...
export default function CompanionRenderer({
  ...
  const [hintVisible] = useState(true);
    ...
    <div style={{ position: "fixed", inset: 0, overflow: "hidden" }}>
      ...
      {!preview && activeHotspots.length > 0 && (
        <div style={{ position: "absolute", top: "1rem", right: "1rem", zIndex: 20 }}>
          <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.75rem", color: "#8B7D72", backgroundColor: "rgba(250,247,244,0.85)", padding: "0.25rem 0.625rem", borderRadius: "2rem", backdropFilter: "blur(4px)" }}>
      ...
      {!preview && hintVisible && (
        <motion.div
          ...
          style={{ position: "absolute", top: "1rem", left: "1rem", zIndex: 20, backgroundColor: "rgba(250,247,244,0.9)", borderRadius: "0.5rem", padding: "0.5rem 0.875rem", backdropFilter: "blur(4px)" }}
        >
